...
## Real local playback validation

Ran the installed, unchanged Apple Development-signed MeetingCapture helper for 18 seconds, 3-second chunks, after confirming no user capture or ASR job. After its `started` event, played public FLEURS Turkish 00.wav once with afplay. Both completed normally; no new permission error, signing, TCC reset, device/volume change or unrelated process termination. Guarded capture sampled tree peak 20,644,992 bytes.

System: seven chunks, 18.06s, 48kHz stereo, nonzero. Mic: six chunks, 17.996s, 48kHz mono, nonzero. Full decoded checks finite. Correlation against the played reference after mono/16k conversion peaked at 0.306 at 0.448s; all offsets >100ms from that peak were <=0.02984 in absolute correlation. This supports presence of the known local playback in the system mix, while other ongoing output may contribute. Mic correlation is weak; nonzero mic establishes signal availability, not reference intelligibility. Numeric artifact: `benchmarks/results/local-playback-capture-2026-09-08.json`. Raw mic/system audio stays locally under ignored `build/source-local-playback-check/`.

This validates short helper capture against known local output, not long-run stability, UI transcript latency, ASR accuracy or permission behavior on another Mac. Previous real meeting's zero system channel remains consistent with lack of local playback then; it is not retroactively repaired.

New fidelity issue for follow-up: raw float system peak 1.0796; after existing mono/resample path peak 1.0567 with 20 samples above unity. `assemble_capture` currently writes PCM_16, so those values cannot be preserved unchanged. CPP export also needs inspection before selecting an amplitude policy; do not silently normalize each segment or claim all clipping fixed by changing one intermediate format. No audio policy change made in this measurement.
