...
ScreenCaptureKit needed the sleeping displays temporarily awakened. The5-minute gate then passed. The15-minute continuity gate also passed with a silent system stream. The30-minute stage also passed with deliberate playback evidence. Inspect the already-running60-minute stage and validate both actual WAV streams and timeline coverage; do not expand the duration ladder after that. Distinguish capture-only costs from inference; retain current pressure/footprint limits. Speaker/headphone echo controls and native UI checks also need hardware access.
