# Meeting OS — Kullanım kılavuzu (1.2.78)
...
- **Ekranda panel yok (1.2.78+):** kayıt sırasında hiçbir pencere ya da panel görünmez (11 Eylül: yüzen panel canlı Zoom görüşmesinde odadakilerce görüldü ve kaldırıldı). Süre ve “kayıt sürüyor” bilgisi menü çubuğu simgesinin menüsünde; bitirmek ⌃⌥R ya da o menüden, an işareti ⌃⌥M.
...
- **Sürüm satırı:** tek satır — “Sürüm 1.2.78 · güncel” ve mini **Güncelleme ara**; yeni sürüm varsa “Güncelle ve yeniden başlat” (Zoom açıkken “Güncelleme toplantı bitince”).
...
**Ekip bilgi tabanı kendiliğinden bağlanır (1.2.78+):** ekip klasörü seçmeseniz de aynı OpenRouter anahtarıyla kurulan Mac'ler aynı ekiptir; öğretilen kelimeler, ses profilleri, sözlük ve raporlar arka planda eşitlenir, sunucu kapalıyken her şey yerelden çalışır (ayrıntı: `docs/EKIP.md` → “Ekip bilgisi buluta gider”).

**Ekibe katılmak bir bağlantıdır (1.2.78+):** Ayarlar (⌘,) → Sesler ve sözlük → **Ekip** kartının ilk satırı her zaman etkin hedefi yazar (“Ekip bulutu · `a1b2c3` · 3 Mac · son eşitleme 14:20”, “Ekip klasörü · ~/Ekip” ya da “Kapalı”), altındaki **Davet bağlantısını kopyala** ise `meetingos://join?team=…` bağlantısını panoya alır; isteğe bağlı kutu işaretlenirse davete OpenRouter anahtarı da girer (o kişinin bulut kullanımını siz ödersiniz). Bağlantı Slack/WhatsApp’tan gönderilir, ekip arkadaşı **tıklar** ve uygulama açılıp ekibe katılır — **terminal, klasör seçimi, adres yapıştırma yok**. Bağlantıyı kabul etmeyen uygulamalar için **Davet dosyasını kaydet…** aynı daveti `.meetingos-invite` dosyası olarak verir (çift tıklamak yeter); zaten anahtarı olan bir Mac’te davetteki anahtar **kullanılmaz**. Yapıştırarak katılmak da mümkün: açılış ekranındaki “Davet bağlantısını buraya yapıştırın” alanı ya da Ekip kartındaki **Davet yapıştır…**. Üç paylaşım anahtarı (sözlük, kelimeler, profiller) artık hedef bulut da olsa klasör de olsa etkindir; yalnız hiçbir hedef yokken kapalıdır. Terminal yolu **Gelişmiş** olarak duruyor: `meeting_os team invite [--with-key]` / `team join <bağlantı | dosya | belirteç>`.
