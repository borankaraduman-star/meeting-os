# Meeting OS — Kullanım kılavuzu (1.2.85)
...
- **Ekranda panel yok (1.2.85+):** kayıt sırasında hiçbir pencere ya da panel görünmez (11 Eylül: yüzen panel canlı Zoom görüşmesinde odadakilerce görüldü ve kaldırıldı). Süre ve “kayıt sürüyor” bilgisi menü çubuğu simgesinin menüsünde; bitirmek ⌃⌥R ya da o menüden, an işareti ⌃⌥M.
- **Göze batmama (pencereler paylaşılan ekranda görünmez):** Göze batma açıkken (varsayılan) ve ekranda bir Zoom toplantısı ya da ekran paylaşımı varken Meeting OS pencereleri `NSWindow.sharingType = .none` ile **ekran yakalama akışının tamamen dışında** kalır: paylaşılan ekranı gören kimse transkripti, özeti ya da isimleri göremez. Aynı ayar kendi ekran görüntülerinizi de engellediği için yalnız toplantı süresince uygulanır — toplantı dışında ⌘⇧4 her zamanki gibi çalışır.
  - **Kanıt (bu Mac'te):** `sh scripts/verify-privacy.sh` — uygulama açıkken, ekran kilidi kapalıyken çalıştırın. Betik pencereyi öne alır, korumayı bir kapatıp bir açar, iki ekran görüntüsü alır ve pencerenin dikdörtgeni içindeki pikselleri karşılaştırır; **GEÇTİ** = açık hâlde o dikdörtgende uygulama içeriği yok, ekranın geri kalanı ise değişmemiş. Ayrıntı: `docs/BUNDLE.md` → Göze batmama kanıtı.
  - **Gerçek kabul ölçütü hâlâ açık:** kanıtın son adımı ikinci bir Mac'ten izlenen canlı bir Zoom paylaşımıdır (Codex incelemesi P0 #7). Bu ölçülene kadar “garanti” değil, “ölçülmüş davranış” deyin.
...
- Kayıt sürerken kenar çubuğunda **An · Karar · Görev · Sonra** düğmeleri çıkar; menü çubuğu menüsünde yalnız **An** ve **Karar** vardır, Görev ve Sonra için kısayolu ya da kenar çubuğunu kullanın.
...
- Ekran uykusu engellenir; ses akışı koparsa (uyku/uyanma, ekran değişimi) ya da 20 sn ses gelmezse yardımcı akışı kendiliğinden yeniden kurar (3 deneme). Disk eşikleri toplantı uzadıkça büyür: kayıt, sonunda birleştirmenin isteyeceği yer (kaynak başına saniyede 64 KB + 200 MB, en az 400 MB) kalmayınca durur; bunun üç katına (en az 3 GB) inince uyarır.
...
3. Özet, kararlar, riskler, açık sorular ve görevler gpt-4.1-mini ile çıkarılır (Ayarlar → Sistem’den değiştirilebilir; seçilen model yanıt veremezse gpt-4.1-mini devreye girer; kıyas: docs/BENCHMARK.md); her madde kaynak konuşmayla gelir, kaynağı doğrulanamayan alıntı atılır.
...
- **Sürüm satırı:** tek satır — “Sürüm 1.2.85 · güncel” ve mini **Güncelleme ara**; yeni sürüm varsa “Güncelle ve yeniden başlat” (Zoom açıkken “Güncelleme toplantı bitince”).
...
- **Konuşanı adlandır** tek soru sorar: ismi yazın (ya da rozetlerden seçin — rozetler bu sese benzerliğe göre sıralıdır), **kapsamı** seçin, **Adlandır**’a basın (⌘↩). Seçtiğiniz kapsamın altındaki tek satır tam olarak ne olacağını söyler; işlem bitince Son durum satırı neyin öğrenildiğini yazar. Metin düzeltme, temiz ses onayı (bu bölümden profil kaydeder) ve **Neden bu isim?** (tek cümle; puanlar ipucunda) **Gelişmiş** altındadır.
  - **Bu konuşmacının tamamı** (varsayılan): bu toplantıdaki bütün bölümleri bu adı alır **ve ses profili öğrenir** — kişi sonraki toplantılarda kendiliğinden tanınır.
  - **Yalnız bu bölüm**: konuşmacı doğru ama **tek bir cümle** başka birine aitse. Sadece o bölüm yeni adı alır, konuşmacının geri kalanı ve profili olduğu gibi kalır, kimse “yanlış” sayılmaz; bölüm en az 6 sn temiz konuşmaysa doğru kişinin profili ondan öğrenir. Paragraf birden çok bölümden oluşuyorsa hangi bölüm olduğunu hemen altındaki menüden seçin. Sonraki küme adlandırmaları bu bölümü atlar.
  - **Yalnız bu toplantıda**: öğrenme yok. İsim bu toplantının bütün bölümlerine yazılır, ses profili oluşmaz.
- **Kelime düzeltme ve öğretme:** Yanlış yazılmış bir kelimeyi **transkriptte kelimenin kendisine tıklayarak** düzeltirsiniz (adlandırma penceresinde kelime kutusu yoktur). Açılan balonda doğrusunu yazıp **Düzelt ve öğret** derseniz, düzeltme o toplantıdaki *bütün* geçtiği yerlere hemen uygulanır (büyük/küçük harf korunur, “Trendyoll’a” → “Trendyol’a” gibi ekler yerinde kalır) ve kelime öğrenilir. Konuşmacı adlandırmada olduğu gibi bir kez yeter: sonraki toplantılarda **aynı yazım** kendiliğinden düzeltilir; doğru yazım sözlüğe de eklenir, böylece bulut modeli bir dahakine baştan doğru yazma şansı bulur. **Yakın yazımlar** (bir harf farkı: “Trendiyol”) kendiliğinden değiştirilmez — Kontrol’e “muhtemelen” önerisi olarak gelir; onaylayınca hem düzelir hem öğrenilir. Nedeni ölçüldü: gerçek toplantılarınızda kelimelerin %42,8’i başka bir gerçek kelimeye bir harf uzaklıkta, yani “Ayşe → Ayşen” öğretmek “Aynen” kelimesini de değiştiriyordu. Doğru yazılmış çekimli hâller (“Trendyola”) ve sözlükteki başka terimler değiştirilmez. Öğrenilen kelimeleri **Ayarlar → Sesler ve sözlük** altında görür (satırda “3 kez tekrar etti, hepsi düzeltildi” gibi bir not varsa, kelime öğretildikten sonra ham transkriptte yine yanlış çıkmış demektir), oradan **Unut** ile geri alırsınız — unutulan kelime, düzelttiği bütün cümleleri eski hâline döndürür. Ekip klasörü seçiliyse öğrettiğiniz kelime ekip arkadaşlarınıza da geçer (ve onlarınki size): listede öğreten Mac’in adıyla görünür, **Kapat** ile yalnız sizde susar, **Unut** yalnız kendi öğrettiklerinizde çıkar.
...
- **Her maddenin sağında ⋯ menüsü** (özet, karar, risk, açık soru): **Düzelt** (satırın altında metin alanı açılır; **⌘↩** kaydeder, Esc vazgeçer), **Kaldır** (isteğe bağlı neden: **Yanlış · Tekrar · Gereksiz ayrıntı**), **Doğru** (anahtar; ikinci basış işareti kaldırır). Düzelttiğiniz madde “düzeltildi”, onayladığınız madde “doğru” çipiyle görünür.
- Bunlar **modelin çıktısının üstünde ayrı bir katmandır**: analiz metni hiç değişmez, sizin cümleniz onun üstünde gösterilir. Menüden **Model cümlesine dön** ile ne yazdığını her zaman geri alabilirsiniz. Bu metin yalnız bu Mac’te durur — rapora, tanılama dosyasına veya ekibe hiçbir koşulda gitmez.
- **Kaldırdığınız madde silinmez.** Bölümün altında küçük bir **“2 madde kaldırıldı · göster”** satırı çıkar; açıp **Geri getir** diyebilirsiniz.
- **Yeniden analiz kararınızı sessizce ezmez.** Her maddenin, yeniden analizde de yaşayan kalıcı bir kimliği vardır: aynı cümle ya da aynı kaynağa dayanan yeniden yazılmış hâli aynı maddedir, düzeltmeniz ona yapışır. Eşleşme şüpheliyse taşınmaz — bir “yanlış” işareti başka bir iddianın üstüne düşmemelidir. Eşleşmeyen değişiklikler bölümün altında **“N değişikliğiniz bu analizde eşleşmedi”** olarak görünür; **Unut** ile kapatırsınız.
- **Dışa aktarma sizin gördüğünüzü aktarır:** düzelttiğiniz cümle gider, kaldırdığınız madde gitmez. Panoya kopyalama, dosyaya kaydetme ve paylaşım önizlemesi aynı görünümü kullanır.
- **Uygulama bu kararlardan üç küçük tercih öğrenir** (1.2.86): ne kadar ayrıntı, ne kadar uzun madde, tekrarları ne kadar birleştirsin. Bir tercih ancak **en az üç ayrı toplantıda** aynı yönde davrandıysanız oluşur; sayı, ad veya olumsuzluk düzelten bir değişiklik **olgu düzeltmesi** sayılır ve üslup tercihine hiç katılmaz. Öğrenilen şey bir sonraki analizin istemine **tek bir sabit Türkçe cümle** olarak eklenir (“maddeler kısa olsun (en fazla 20 kelime); aynı konudaki tekrarları birleştir”) ve özet uzunluğu hedefini en çok dörtte bir oynatır. **Sizin yazdığınız hiçbir cümle buluta gitmez** — giden metin sabit bir şablondan seçilir, ek model çağrısı yoktur, ek maliyet yoktur. Görevlerim’de “Çıkarım hatası” diye işaretlediğiniz düzeltmeler de aynı şekilde **hata türüne** göre sayılır (aktarılan söz, koşullu söz, mikrofon yankısı, tarih yorumlama); bir tür tekrar tekrar yanlış çıkıyorsa uygulama o türdeki **yeni** maddeleri Kontrol’e düşürür — sahibi ya da tarihi kendiliğinden hiçbir zaman değiştirmez ve kişiye bağlı bir kural (“bu işi hep Ayşe yapar”) asla üretmez. İkisini de `meeting_os quality preferences` ve `meeting_os quality task-errors` ile görebilirsiniz.
...
- Şüpheli yerler sırayla: onay bekleyen isim (tek tık Onayla), isimsiz konuşmacı (takvim katılımcı çipleriyle adlandırma), çakışan konuşma, kısa sesle tanıma, sahibi belirsiz görev, **kontrol bekleyen görev**, sözlük düzeltmesi.
- **Her maddenin altında üç düğme: Doğru · Düzelt… · Geç.** Verdiğiniz cevap, maddenin çıktığı **kaynak sürümüne** yazılır: **Doğru** dediğiniz madde o transkript/analiz için bir daha gelmez. **Düzelt…** sizi düzelteceğiniz yere götürür (bölüm düzenleyicisi ya da Görevlerim) ve düzeltildi olarak kapatır. **Geç** onay değildir — maddeyi yalnız bu sürüm için gizler ve ayrı sayılır; kuyruğu geçerek kısaltmak kaliteyi artırmaz.
- **Kaynak değişirse madde geri gelir.** Transkripti düzelttiğinizde ya da yeni bir analiz çalıştığınızda o yer artık eskisi değildir; eski cevap eski metne aitti, soru yeniden sorulur.
- **Kontrol bekleyen görev**: analizin kendisinin şüpheli bulduğu, sahibi dolu görevler (şüpheli sahip, çakışan kaynak, tek sözün iki okuması). Eskiden yalnız Görevlerim’de bir rozetti; artık aynı üç düğmeyle kapatılabilen bir iş.
...
- **Ekip yazımı** maddeleri (`word_conflict`): iki ekip arkadaşınız aynı kelimeyi farklı öğretmiş ve sizde kendi kuralınız yok. Madde iki düğme gösterir (“Trendyol” / “Trendyol A.Ş.”); seçtiğiniz yazım bu Mac’te öğrenilir, ekip klasöründeki satırlara dokunulmaz. Soru kelime başına bir kere sorulur, bölüm başına değil.
- **Kelime** maddeleri: bu toplantıda geçen ama öğrettiğiniz bir kelimenin ya da sözlükteki bir terimin *yakın yazımı* olan sözcükler (“Kelime: “Trendiyol” muhtemelen “Trendyol” · öğretilen kelime / sözlük terimi”). Kontrol’ün eskiden göremediği tek şey buydu: modelin kendinden emin yazdığı yanlış kelime, doğru olanla aynı görünür. **Düzelt ve öğret** kelimeyi hem burada hem bundan sonra düzeltir, **Yoksay** (“Bu doğru”) o kelimeyi *hiçbir* toplantıda bir daha sormaz. Liste dar tutulur: yalnız bir harf farkı, sözlük tarafında yalnız altı harften uzun terimler; sık geçen Türkçe kelimeler ve zaten doğru yazılmış terimler girmez.
...
- **Kişi tanıma kalibrasyonu** (1.2.85): uygulama ses eşleştirme eşiğini kendiliğinden değiştirmez, ama kendi toplantılarınızda ölçer. Boştaki saatlik bakım geçişi, günde en fazla bir kez, küçük bir eşik/marj ızgarasını **yalnız sizin adlandırdığınız** kümeler üzerinde ve **zaman sıralı** koşturur (bir toplantı yalnız kendisinden önce var olan kanıtı kullanabilir). En az 20 doğrulanmış küme biriktiğinde Ayarlar → **Kurulum durumu** kartının kalite satırı öneriyi yazar — “kalibrasyon önerisi: eşik 0.85 (+2 doğru, 0 yanlış, n=24)” —, altındaysa yalnız “kalibrasyon: veri yetersiz (n=…)” der. Öneri **uygulanmaz**: istersen Terminal’de `meeting_os quality calibrate --apply` ile yazarsın, doğrulanmış aralık dışındaki bir değer yok sayılır. Aynı kartın ekip satırı da ekip profillerinin gerçekte ne kazandırdığını söyler (“ekipten gelen profiller: +2 doğru / −1 yanlış”); yalnız ekipten bilinen bir kişi, uygulama çok emin olmadıkça ad olarak yazılmaz, **öneri** olarak sorulur.
...
![Koyu tema ve mavi vurgu seçili hâli; menü çubuğu menüsü de temayı izler.](img/theme.jpg)

*Koyu tema ve mavi vurgu seçili hâli; menü çubuğu menüsü de temayı izler.*
...
- **Sesler ve sözlük — kim konuşuyor, sözcükler nasıl yazılıyor:** kayıtlı ses profilleri (örnekleri dinleme/silme, yeniden adlandırma, profil silme); sözlük (kişi adları / özel terimler, satır başına bir; **Sözlüğü kaydet**), `glossary.jsonl` içe aktarma; **Ekip** kartı (etkin hedef satırı, davet bağlantısı/dosyası, davet yapıştırma, “Şimdi eşitle”, Gelişmiş altında ekip klasörü) ve üç anahtar — **Sözlüğü ekiple paylaş**, **Öğretilen kelimeleri ekiple paylaş**, **Ses profillerimi ekiple paylaş (kişi adı + ses vektörü; ses kaydı değil)**. Üçü de hedef ne olursa olsun (bulut ya da klasör) çalışır, yalnız hiçbir hedef yokken kapalıdır. Son ikisi varsayılan **açıktır**: ekip klasörü ortak bilgi tabanıdır, herkesin adlandırması ve kelime düzeltmesi herkeste birikir. Kapalıyken yalnız bu Mac tanır; açınca ekip klasörüne yazılır ve ekip arkadaşlarınız bu kişileri ilk toplantıda tanır. **Ses kaydı ve transkript hiç paylaşılmaz.** Ekip klasörünü ilk seçtiğinizde o ana kadar öğrettiğiniz kelimeler ve kaydettiğiniz profillerin **tamamı** bir kez yayımlanır; başlığın yanındaki “ekipten N profil, M kelime” satırı ne kadarının ekipten geldiğini söyler. Ekipten gelen bir kelime, öğrenilen kelimeler listesinde öğreten Mac’in adıyla görünür ve satırındaki **Kapat** ile yalnız bu Mac’te susturulur; ekipten gelen bir ses örneği kişi kartında “ekipten (mac-adı)” diye durur, **Profili sil** onu da siler ve geri gelmesini engeller.
- **Sistem — makinenin kendi kendine yaptıkları:** **Yazıya çevirme** modu ve model (OpenRouter / Yerel; eskiden kenar çubuğundaydı); **OpenRouter anahtarı**; **Depolama** (toplam kullanım **Ses / Veritabanı / Ekip önbelleği / Günlükler** olarak ayrılır, en büyük toplantılar, **Sesleri sıkıştır**); bulut maliyeti (bu ay / toplam); **Güncelleme ve raporlar** (Şimdi kontrol et); **Kurulum durumu** (izinler, anahtar, sözlük, sürüm, rapor klasörü). Seyrek kullanılanlar **Gelişmiş** katında: **Eski toplantıların sesi** silinmesin / 14 / 30 / 60 / 90 gün sonra ve önizlemeli **Eski sesleri temizle**, **Eski toplantıların yazısı** silinmesin / 90 / 180 / 365 / 730 gün sonra, açılışta kendiliğinden güncelle, bulut hatasında boşta yeniden dene, **Ölçülmüş iyileştirmeler kendiliğinden uygulansın** (varsayılan kapalı), her toplantıdan sonra teşhis raporu, raporlara transkript ekleme, rapor klasörü, **Öz-test**, uygulama yoklama gecikmesi p50/p95.
...
**Metin saklama süresi — toplantının kendisi.** **Eski toplantıların yazısı** varsayılan olarak **silinmesin** (0) durumundadır: ses yeniden üretilemez ama yazı *toplantının kendisidir*, ve kimse bir ayarı okumadığı için transkriptini kaybetmemelidir. Bir süre seçerseniz o süre dolan toplantı **tümüyle** silinir — transkript, özet, kararlar, sorular, riskler, görevler ve taslaklar, Hafıza’daki kayıtları, o toplantının teşhis raporu (ekip klasöründeki kopyası dâhil) ve başlığı; ses zaten kendi süresinde gitmiştir. Elle **Sil** ile aynı yoldan gider, yani geride yetim bir dosya kalmaz. Dokunulmayanlar: ses profilleri ve öğretilmiş kelimeler. Bu ayarın anlamı bir kriz senaryosudur: **önceden belirlenmiş bir süre, iş işten geçtikten sonra toplu silmeye çalışmaktan daha güvenlidir.** Temizlik saatte bir, yalnız kayıt yokken çalışır; kaydedilen, işlenen, tamamlanmamış, bulutta hata almış ve **Sesi koru** işaretli toplantılara hiç dokunmaz. Depolama kartı, ilk toplantı silinmeye üç gün kala “N toplantının yazısı … tümüyle silinecek” satırını gösterir.

**Ölçülmüş iyileştirmeler ve geri alma** (1.2.85). Ayarlar → Sistem → Gelişmiş’teki **“Ölçülmüş iyileştirmeler kendiliğinden uygulansın (yalnız yerel ayarlar; kayıt sırasında asla; her değişiklik geri alınabilir)”** anahtarı **varsayılan kapalıdır**. Uygulama boştayken, **günde en fazla bir kez** ve **kayıt ya da iş sürerken hiç çalışmadan** üç ucuz adayı kendi verinizle ölçer: ses eşleştirme eşiği, Kontrol kuyruğunun sıralaması ve öğretilen kelimelerin ipucu sırası. Ölçüm **buluta hiçbir şey göndermez** ve sonuçları yalnız `quality/experiments.jsonl` dosyasına yazar (7 gün / 20 MB); görevlere, kişi profillerine ve öğretilen kelimelere hiç dokunmaz. Anahtar kapalıyken bulduğunu yalnız yazar: Kurulum durumu kartının kalite satırı “kalibrasyon önerisi: eşik 0.85 (+2 doğru, 0 yanlış, n=24) **· otomatik uygulama kapalı**” der ve öneriyi siz uygularsınız. Açıkken, **önceden ilan edilmiş hedefi tutan** (doğru sayısı artıyor, yanlış artmıyor, en az 20 gözlem) ve doğrulanmış aralıkta kalan aday bir **politika sürümü** olarak yürürlüğe girer; kart o sürümü yazar (“politika v2 · eşik 0.85 · marj 0.05”). Her değişiklik tek adımda geri alınır:

```bash
meeting_os quality policy              # yürürlükteki sürüm, hangi kanıtla ve bütün geçmişi
meeting_os quality policy --rollback   # bir adım geri
```

Bir Kontrol maddesine **dokunmamak ya da “Geç” demek onay sayılmaz**: ölçüm yalnız sizin gerçekten verdiğiniz kararları kullanır.

**Ekip önbelleği ne kadar büyür.** Ekip arkadaşlarınızın Mac’lerinden inen teşhis raporları `team/` altında birikir ve en fazla **300 rapor** ya da **50 MB** tutulur; sınırı aşan en eski raporlar silinir ve bir daha indirilmez. Kendi raporlarınıza, nabız dosyalarına, ekip sözlüğüne, öğretilen kelimelere ve ses profillerine bu sınır uygulanmaz.
...
**Ekip bilgi tabanı kendiliğinden bağlanır (1.2.85+):** ekip klasörü seçmeseniz de aynı OpenRouter anahtarıyla kurulan Mac'ler aynı ekiptir; öğretilen kelimeler, ses profilleri, sözlük ve raporlar arka planda eşitlenir, sunucu kapalıyken her şey yerelden çalışır (ayrıntı: `docs/EKIP.md` → “Ekip bilgisi buluta gider”).
**Yazım ipucu sıralı gider (1.2.85+).** Modele gönderilen ipucu 900 karakterle sınırlı ve bu sınır değişmedi; değişen, bu karakterlerin **hangi kelimelere** harcandığı. Sıra: (1) öğrettiğiniz ama ham transkriptte **yine yanlış çıkan** kelimeler, (2) son 30 günde öğrettikleriniz, (3) yerelde doğruladığınız diğer yazımlar (öğretilenler ve “bu doğru” dedikleriniz), (4) ekip arkadaşlarınızın öğrettikleri, (5) sözlük terimleri (kaç kaynakta geçtiğine göre), (6) `vocabulary.txt` listeniz. Aynı kelime iki kez yer kaplamaz. Kaç terimin girdiği ve kaçının bütçeye sığmadığı toplantı raporunda **sayı olarak** durur — kelimelerin kendisi bu Mac’te kalır. Terminalden bakmak için: `meeting_os glossary hint`.

**Öğrettiğiniz kelime işe yaradı mı?** Ayarlar → Sesler ve sözlük → Öğrenilen kelimeler listesinde, kelime öğretildikten **sonra** kaydedilen toplantılarda ham transkript eski yazımı yine yazmışsa satırda görünür: “3 kez tekrar etti, hepsi düzeltildi” ya da turuncu olarak “3 kez tekrar etti, 2 tanesi düzeltilmedi”. Bir toplantı bir kez sayılır. Bu sayı yalnız bu Mac’te durur; ekibe ve raporlara yalnız toplam oran çıkar, hiçbir kelime çıkmaz.

**İki ekip arkadaşı aynı kelimeyi farklı yazdıysa** hiçbiri kendiliğinden uygulanmaz (eskiden en son yazan kazanıyordu). Kontrol’e tek bir madde gelir: **“Ekipte iki yazım: X / Y — hangisi?”**; hangisine basarsanız o yazım **bu Mac’te** öğrenilir ve bundan sonra sessizce kazanır. Ekip klasöründeki dosyalara dokunulmaz, kimsenin kelimesi yasaklanmaz, oylama yapılmaz. Zaten kendi öğrettiğiniz bir kelime varsa soru hiç sorulmaz: yerel yazım her zaman önceliklidir.

**Ekip bilgi tabanı kendiliğinden bağlanır (1.2.82+):** ekip klasörü seçmeseniz de aynı OpenRouter anahtarıyla kurulan Mac'ler aynı ekiptir; öğretilen kelimeler, ses profilleri, sözlük ve raporlar arka planda eşitlenir, sunucu kapalıyken her şey yerelden çalışır (ayrıntı: `docs/EKIP.md` → “Ekip bilgisi buluta gider”).

**Ekibe katılmak bir bağlantıdır (1.2.85+):** Ayarlar (⌘,) → Sesler ve sözlük → **Ekip** kartının ilk satırı her zaman etkin hedefi yazar (“Ekip bulutu · `a1b2c3` · 3 Mac · son eşitleme 14:20”, “Ekip klasörü · ~/Ekip” ya da “Kapalı”), altındaki **Davet bağlantısını kopyala** ise `meetingos://join?team=…` bağlantısını panoya alır; isteğe bağlı kutu işaretlenirse davete OpenRouter anahtarı da girer (o kişinin bulut kullanımını siz ödersiniz). Bağlantı Slack/WhatsApp’tan gönderilir, ekip arkadaşı **tıklar** ve uygulama açılıp ekibe katılır — **terminal, klasör seçimi, adres yapıştırma yok**. Bağlantıyı kabul etmeyen uygulamalar için **Davet dosyasını kaydet…** aynı daveti `.meetingos-invite` dosyası olarak verir (çift tıklamak yeter); zaten anahtarı olan bir Mac’te davetteki anahtar **kullanılmaz**. Yapıştırarak katılmak da mümkün: açılış ekranındaki “Davet bağlantısını buraya yapıştırın” alanı ya da Ekip kartındaki **Davet yapıştır…**. Üç paylaşım anahtarı (sözlük, kelimeler, profiller) artık hedef bulut da olsa klasör de olsa etkindir; yalnız hiçbir hedef yokken kapalıdır. Terminal yolu **Gelişmiş** olarak duruyor: `meeting_os team invite [--with-key]` / `team join <bağlantı | dosya | belirteç>`.

**Ekip klasörü** (Ayarlar → Sesler ve sözlük → Ekip → Gelişmiş → Ekip klasörü seç…): iCloud tek Apple Kimliğine bağlı olduğundan ekip için ortak bir klasör (paylaşılan disk, Drive, Dropbox) seçilir. Orası ekibin ortak bilgi tabanıdır: sözlüğün yanında öğretilen kelimeler (`team-words.jsonl`) ve ses profilleri (`profiles/<mac-adı>.jsonl`) de orada birikir — ayrıntı ve gizlilik tablosu `docs/EKIP.md` → “Ekip bilgisi”. Sözlük okunurken yerel dosya → iCloud → ekip dosyası sırasıyla bakılır, terimi ilk tanımlayan kazanır; içe aktarma ekip dosyasını **birleştirerek** yazar (önce yeniden okur, geçici dosyaya yazıp yerine taşır), böylece aynı anda yazan başka bir Mac’in terimleri silinmez. Ekip klasörü doluyken teşhis raporları da kişisel klasör yerine `<ekip klasörü>/reports/<mac-adı>/` altına yazılır. Ses kaydı, transkript metni, toplantı adı ve toplantı numarası bu klasöre hiç girmez.
