...
The native sidebar now offers “Tanılama raporu kaydet”. It chooses a new JSON filename and invokes a separate local bridge request; it does not replace the active job, its progress file or last-job log. The bridge handles diagnostics before opening Store/Memory and includes only the active progress path when available. Existing destinations remain protected by the no-overwrite exporter even if the save panel offers a replacement. Export can be used while a job is running. Bridge privacy tests and native compilation passed; real save-panel interaction/visual QA remains unverified.
