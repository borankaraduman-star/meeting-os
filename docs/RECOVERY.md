...
## C5 normal capture completion

Native recording now requests `record --output RECEIPT.json`. After successful capture completion, the CLI atomically publishes a private0600 receipt containing the original meeting ID, exact capture directory, final status and finalized-chunk count. Native code accepts only a provisional receipt for its own directory with a nonempty ID and positive chunk count, then launches guarded retry for that ID. It no longer calls the old finalize command or creates a separate final meeting on normal native recording completion.

The handoff does not depend on library polling or the current selection, and completion does not force navigation to another meeting. A missing/mismatched/canceled/empty receipt does not trigger inference. Capture failures publish no success receipt. A quit request prevents new retry or analysis work; captured records can be completed from the archive later. The existing16GiB automatic-analysis deferral is preserved. Direct CLI finalize remains available for legacy workflows, but native automatic/manual recovery now share retry.

Tests cover atomic receipt identity/permissions, failed capture with no receipt, Swift receipt acceptance/rejection, and a fake capture helper through actual record/retry CLI orchestration with one database meeting before/after completion. Real model quality, hardware capture and GUI clicks are not simulated by these tests. Existing old duplicate records are not automatically deleted or merged.
