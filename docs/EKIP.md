...
- [ ] Ayarlar → **Sistem → Depolama**: kayıt sırasında ses geçici olarak ≈2 GB/saat yer kaplar, sıkıştırıldıktan sonra ≈150 MB/saat kalır (ölçülen bir toplantı: 41 dakika = 82 MB). Diskte birkaç GB boş tutun; “Eski toplantıların sesi” seçeneğini (varsayılan 30 gün) kendinize göre ayarlayın.
...
3. Hâlâ olmuyorsa Boran’a şunu gönderin (sırayla, elinizde ne varsa):
   ...
   - Ayarlar → Sistem → Kurulum durumu → **Öz-test** düğmesine basıp sonucun ekran görüntüsü.
