...
## Owned-audio checkpoint review

Actual Claude Code Sonnet reviewed the narrow worker ownership diff with no blockers and proposed held-out Turkish/English code-switch evaluation before model promotion. Its wording “3-point WER gain” is incorrect: observed difference is three word edits, or1.23 percentage points (14/244 vs17/244). Synthetic speech may exercise the harness but cannot substitute for held-out human meeting accuracy. Next accuracy acceptance requires human-reviewed references, separate name/term precision and recall, unchanged scoring, and single-job resource-guarded inference. No new model promotion or automatic training was performed. Review stays local at build/benchmarks/claude-owned-audio-review.md.
