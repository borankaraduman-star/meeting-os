...
import json
import re
from .intelligence import parse_json
...
def build_document(store, mid, kind, llm, segment_ids=None, glossary=None):
    ...
    context = {'document_type': title, 'headings': headings, 'meeting': meeting['title'], 'analysis': analysis,
               'sources': [{'segment_id': r['id'], 'speaker': r.get('speaker_name') or r['speaker'], 'text': (r['text'] or '')[:1800]} for r in rows[:120]]}
    ...
    if cited:
        ...
        for sid in cited[:30]:
            r = by_id[sid]; lines.append(f'- #{sid} · {r.get("speaker_name") or r["speaker"]} · {int((r["start"] or 0)//60):02d}:{int((r["start"] or 0)%60):02d}: “{(r["text"] or "")[:220]}”')
