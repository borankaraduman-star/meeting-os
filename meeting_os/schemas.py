...
def analysis_schema(ids,summary_only=False,summary_max=12):
    ...
    # `minItems` on the summary only: a meeting may genuinely have no decision, risk, question or task, but a
    # transcript with no summary at all is an empty answer, and an empty answer used to validate, be saved and
    # then be served from the cache forever because it was not stale (audit 2026-09-13 #5).
    return {'type':'object','properties':{key:{'type':'array','maxItems':summary_max if key=='summary' else 8,'items':action if key=='actions' else item,**({'minItems':1} if key=='summary' else {})} for key in keys},'required':keys,'additionalProperties':False}
