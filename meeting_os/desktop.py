...
def dispatch(request, db=None):
    ...
    with contextlib.closing(Store(db or DATA_DIR/'meeting-os.sqlite')) as store:
        ...
        if action=='brief':
            ...
            from .reports import settings_owner as _owner_setting
            brief=build_brief(store,request.get('title') or '',request.get('attendees') or [],owner=_owner_setting(DATA_DIR if db is None else Path(db).parent));text=render_brief(brief)
