        ...
        try:
            if args.command in ('analyze','prepare','ask','handoff','actions','action-update','search','mcp'):
                ...
                if args.action=='report': output(quality.report(store))
                    ...
                    # Derived from this Mac's own summary decisions; the analysis reads the saved file.
                    from . import preferences as P
                    record=P.derive(store);P.save(args.db.parent,record)
                    output({**record,'line':P.line(record)})
                elif args.action=='task-errors':
                    from . import task_errors as TE
                    record=TE.measure(store);TE.save(args.db.parent,record)
                    output({**record,'line':TE.line(record)})
                    ...
                    summary,full=quality.replay(store,args.db.parent,identity=args.identity or not picked,text=args.text or not picked,timeline=args.timeline)
