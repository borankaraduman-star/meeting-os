...
def parser():
    ...
    q=sub.add_parser('quality',help='Personal quality set from your corrections'); q.add_argument('action',choices=['report','compare','replay','daily','calibrate','preferences','task-errors']); q.add_argument('--model',action='append',default=[]); q.add_argument('--limit',type=int,default=20); q.add_argument('--allow-upload',action='store_true'); q.add_argument('--identity',action='store_true',help='replay: voice matching only'); q.add_argument('--text',action='store_true',help='replay: text corrections only'); q.add_argument('--timeline',action='store_true',help='replay: time-ordered cold start (only evidence older than the meeting)'); q.add_argument('--day',help='daily: the day to measure (YYYY-MM-DD), default today'); q.add_argument('--json',action='store_true',help='replay: print the full result, not the summary'); q.add_argument('--apply',action='store_true',help='calibrate: write the recommended identity threshold/margin into settings.json')
